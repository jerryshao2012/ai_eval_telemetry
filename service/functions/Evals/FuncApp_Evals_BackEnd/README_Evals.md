...
1. **`ingest_telemetry` (HTTP Trigger)**
   ...
   - The function now enforces a tighter contract:
     ...
     - enriches each accepted event with `id`, `ingestionId`, and `receivedAt` if they are not already present
   - The HTTP response includes `ingestionId` so downstream Event Hub and Cosmos activity can be correlated with the original request.
...
3. **No stable correlation metadata was added to events**
   ...
   - Implemented: events now default `id`, `ingestionId`, and `receivedAt` so requests, Event Hub messages, and Cosmos documents can be tied together.
